<script lang="ts">
	import { type IconDefinition, faCaretRight } from '@fortawesome/free-solid-svg-icons';
	...
	let { icon, name, onclick, context, disabled = false, selected = false, open }: Props = $props();
...
<tbody bind:this={element}>
	...
	<tr class="item" onclick={interact} onmousemove={hover} class:disabled class:selected>
		<td class="icon">
			{#if icon}<Fa {icon} />{/if}
		</td>
		<td class="name">
